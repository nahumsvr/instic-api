...
- **Uso del Nest CLI**: Para la creación de nuevos componentes (módulos, controladores, servicios, guards, pipes, etc.) es obligatorio utilizar el CLI de Nest (`nest g resource <resource-name> --no-spec`, `nest g controller --no-spec`, etc.). Esto asegura que los archivos se creen con la estructura y nombres correctos y se registren automáticamente en los módulos correspondientes.
